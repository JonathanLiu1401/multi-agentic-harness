...
description: Multi-Agentic Harness - Claude is captain (architect/QA/reviewer); workers do implementation and parallel review across Claude Code provider harnesses (clx for Grok, clg for Gemini, cld for DeepSeek, clo for OpenRouter/Nemotron, clc for Cursor, claude for Anthropic) or native CLI terminals. Trigger for /claude-manages-codex, "delegate to grok", "use the multi-agent harness", "parallelize with subagents", "first mate", "run full parallel /work-checker", or any coding/review task where Claude decides and a worker codes.
---

# Multi-Agentic Harness (internal id: claude-manages-codex)

> **Rename note (2026-07-15, updated 2026-09-16):** this skill is branded the **Multi-Agentic Harness**. Its internal id / MCP tool prefix / install directory remain `claude-manages-codex` for compatibility. Codex is DISABLED.
>
> **Claude Code Provider Harnesses:** The multi-agentic harness can launch workers directly through Claude Code harnesses (`clx`, `clg`, `cld`, `clo`, `clc`, and `claude`) using `start_claude_worker` (headless) or `start_visible_claude_worker` (visible terminal window) by setting `harness="<name>"`. Workers run under the full power of Claude Code (tools, skills, MCP servers, hooks) on third-party provider quotas without consuming Anthropic session limits.
>
> **cursor-agent usage EXHAUSTED (owner 2026-09-14).** Do not fan out with `start_visible_first_mate_cursor_pool` or `start_visible_cursor_worker`. For Cursor models, use `harness="clc"`. Visible terminal fan-out is Grok (`start_visible_first_mate_grok_pool` / `start_visible_grok_worker`) or Claude Code visible workers (`start_visible_claude_worker`).

Use this session's model as captain (Claude on plain `claude`, grok on `clx`, Gemini/agy on `clg`, DeepSeek on `cld`, OpenRouter model on `clo`, Cursor-hosted model on `clc`).

## MANDATORY INVOCATION RULE (owner directive, updated 2026-09-16)
...
3. **Choose the worker harness matched to the task:**
   ...
   - **Cursor workers (`clc`):** Call `start_claude_worker(harness="clc")` / `start_visible_claude_worker(harness="clc")`. 1M context.
   - **Parallel Fan-out / First-Mate Pool:** Call `start_visible_first_mate_claude_pool(harness="<name>")` or `start_visible_first_mate_grok_pool`.
4. For review, inspection, or work-checker tasks: launch workers with `sandbox="read-only"`.
5. For implementation or code-editing tasks: launch workers with `sandbox="workspace-write"` (or `danger-full-access`).
6. Subagent Locality (in-process `Agent` tool) applies ONLY to routine, unprompted background delegation where the user NEVER typed `/claude-manages-codex` and never requested the harness. The moment `/claude-manages-codex` is invoked or loaded, that IS an explicit, mandatory command to offload to external worker harnesses.
7. **NEVER FAN OUT TO CURSOR-AGENT CLI.** cursor-agent usage is exhausted (owner 2026-09-14). Do not call `start_visible_first_mate_cursor_pool` or `start_visible_cursor_worker`. Use `harness="clc"` instead.

## Subagent Locality (routine internal delegation only)
...
- A **plain Claude** session (`~/.claude`, api.anthropic.com) delegates unprompted background tasks through the `Agent` tool with a built-in `subagent_type` (`general-purpose`, `Explore`, `Plan`, `claude`, ...). But when `/claude-manages-codex` is invoked, it MUST offload to worker harnesses (`start_claude_worker` / `start_visible_claude_worker` with `harness="clo"`, `"clx"`, etc., or `start_visible_grok_worker`).
- A **`clx` session** (Claude Code on grok via CLIProxyAPI) uses Agent `subagent_type: "grok"` for routine grok work.
- A **`clg` session** (Claude Code on Gemini via CLIProxyAPI) uses Agent `subagent_type: "agy-gemini-3-8-flash"` for routine agy work.
- A **`cld` session** (Claude Code on DeepSeek via api.deepseek.com) uses Agent `subagent_type: "deepseek"` for routine DeepSeek work.
- A **`clo` session** (Claude Code on OpenRouter) uses Agent `subagent_type: "openrouter"` for routine OpenRouter work.
- A **`clc` session** (Claude Code on Cursor via the local translator on `127.0.0.1:8318`) has no native Agent types.
- Native in-process `Agent` spawns are same-harness only inside their own profile. clx, clg, cld, clo, and clc are not cross-compatible for in-process Agent tools.
- Cross-harness delegation and multi-agent harness tasks ALWAYS use the bridge tools (`start_claude_worker`, `start_visible_claude_worker`, `start_visible_first_mate_claude_pool`, `start_visible_grok_worker`, `start_visible_agy_worker`).

## Which session am I in? (Captain Context)

Check `CLAUDE_CONFIG_DIR` (or `ANTHROPIC_BASE_URL`):

| Captain | Same-family workers | Other-family workers via bridge |
| --- | --- | --- |
| **Plain Claude** (`~/.claude`, api.anthropic.com) | Agent built-in `subagent_type` | `start_claude_worker` / `start_visible_claude_worker` with `harness="clo"|"clx"|"clg"|"cld"|"clc"`; or native CLI tools (`start_visible_grok_worker`, `start_visible_agy_worker`). |
| **clx** (`~/.claude-clx`, grok 500k) | Agent `subagent_type: "grok"` | Bridge tools with `harness="clo"|"clg"|"cld"|"clc"|"claude"`; or native `start_visible_agy_worker`. |
| **clg** (`~/.claude-clg`, Gemini 1M) | Agent `subagent_type: "agy-gemini-3-8-flash"` | Bridge tools with `harness="clo"|"clx"|"cld"|"clc"|"claude"`; or native `start_visible_grok_worker`. |
| **cld** (`~/.claude-cld`, DeepSeek 1M) | Agent `subagent_type: "deepseek"` | Bridge tools with `harness="clo"|"clx"|"clg"|"clc"|"claude"`; or native `start_visible_grok_worker`. |
| **clo** (`~/.claude-clo`, OpenRouter 1M) | Agent `subagent_type: "openrouter"` | Bridge tools with `harness="clx"|"clg"|"cld"|"clc"|"claude"`; or native `start_visible_grok_worker`. |
| **clc** (`~/.claude-clc`, Cursor 1M via `127.0.0.1:8318`) | None | Bridge tools with `harness="clo"|"clx"|"clg"|"cld"|"claude"`; or native `start_visible_grok_worker`. |

Context windows:
- **clx is 500k.**
- **clg, cld, clo, clc are 1M.**
- Send massive context tasks to `clg`, `clo`, or `cld`.

## Mandatory Spawn Path (updated 2026-09-16)
...
| **Gemini / Antigravity native CLI worker** | `start_visible_agy_worker` | `steer_visible_agy_run` |
| **DeepSeek Claude Code worker** | `start_claude_worker(harness="cld")` or `start_visible_claude_worker(harness="cld")` | `steer_claude_run` |
| **Cursor Claude Code worker** | `start_claude_worker(harness="clc")` or `start_visible_claude_worker(harness="clc")` | `steer_claude_run` |
| **Claude (Anthropic) worker** | `start_claude_worker(harness="claude")` or `start_visible_claude_worker(harness="claude")` | `steer_claude_run` |
| **First-Mate Pool fan-out** | `start_visible_first_mate_claude_pool(harness="...")` or `start_visible_first_mate_grok_pool` | `steer_claude_run` / `steer_visible_grok_run` |
...
- **Fanning out with `start_visible_first_mate_cursor_pool` / `start_visible_cursor_worker` (CRITICAL ERROR as of 2026-09-14):** cursor-agent usage is gone. An older section calling cursor the preferred visible worker is stale. Harness and visible-window fan-out is Grok. `cursor_agent.available` (CLI present) is not quota remaining.
- Shelling out to `cursor-agent -p`, the `grok` CLI, or Codex from a Bash call to delegate from a Claude session. Subagent Locality forbids that; use `start_visible_grok_worker` / `start_visible_first_mate_grok_pool` when the user asked for the harness, for visible windows, or for grok agents. Owner-requested grok-to-edit still goes through the MCP tool, with a write sandbox.
...
- **cursor-agent in a visible window (EXHAUSTED 2026-09-14 - do not fan out)** - `start_visible_cursor_worker`, `start_visible_haiku_composed_cursor_worker`, `start_visible_first_mate_cursor_pool`, `steer_visible_cursor_run` remain in the bridge for owner-named revival only. Do not treat `cursor_agent.available` as permission to spawn. Default model `cursor-grok-4.6-xhigh-fast` (grok 4.6 xhigh Max Mode fast, Cursor 1M Max Mode).
...
9. **Call `check_worker_backends` before delegating to a visible-window or headless backend.** If the chosen backend is unavailable, fall back and tell the user why. `cursor_agent.available` means the CLI is installed, not that quota remains: cursor-agent usage is latched exhausted (2026-09-14).

### Claude Code Worker Backends (headless & visible; added 2026-07-18, updated 2026-09-16)
...
Signatures:
- `start_claude_worker(prompt, cwd, title="Claude worker", model="", harness="claude", sandbox="read-only", effort="", session_context="", resume_session_id="", max_budget_usd="", steer_idle_seconds=20, visible=False)`
- `start_visible_claude_worker(prompt, cwd, title="Visible Claude worker", model="", harness="claude", sandbox="read-only", effort="", session_context="", resume_session_id="", max_budget_usd="", steer_idle_seconds=20)`
- `start_visible_first_mate_claude_pool(goal, cwd, scout_areas=None, implementation_items=None, harness="claude", model="", sandbox="read-only", max_workers=6, session_context="", effort="", visible=True)`

Key capabilities:
- `sandbox`: `read-only` strictly enforces no-edit by stripping Write/Edit tools via `--disallowed-tools Write,Edit`, while allowing reading and MCP callbacks without permission prompt hangs. `workspace-write` allows edits.
- `steer_claude_run(run_dir, instruction, ..., interrupt_current_turn=False)` steers an active worker or resumes a completed session under the exact same harness and settings.
- Full run-dir protocol preserved: `events.jsonl`, `display.log`, `status.json`, `captain_reports/`, `captain_help/`, `steer_queue/`.
- MCP tools available inside workers: `submit_captain_report` and `request_captain_help`. Captain answers via `respond_to_captain_help_request`.
- `check_worker_backends` reports availability for all harnesses (`harness_clo`, `harness_clx`, `harness_clg`, `harness_cld`, `harness_clc`, `harness_claude`).
...
1. **Worker Rigor Contract (automatic).** Every grok worker prompt is prepended with a mandatory contract (`_grok_rigor_contract`) that forces the worker to: enumerate 2-3 hypotheses/approaches and the edge/error/boundary cases before coding; adversarially pressure-test its own change; **actually run it end to end and paste the observed output as proof** (a confident "done" without executed evidence is defined as a failure); and report what it did NOT test plus the top 2 ways it could still be wrong. You do not need to add this to your brief - it is always injected - but your `prompt_brief` should still name the concrete acceptance test and the specific scenarios/edge cases you want covered.
  ...
  **A DISCLOSED DEFECT IS NOT A DISCHARGED DEFECT.** The most common way a worker ships broken work is not hiding the problem, it is naming it. It writes the failure into its Risks section, feels the obligation discharged, and ends the turn: "note that this is still not complete". Observed three times in a single session. A worker noted that a lookback window "will mark the start of the next straight after an elbow" and shipped, not noticing the same parameter also capped its true-positive coverage so detection decayed as 1/duration. Its next attempt disclosed a false-positive regression and shipped that too, measured at 74.4% of good travel wrongly flagged. A third correctly diagnosed that deriving dt from clock-fit timestamps reimports jitter into gyro integration, and did nothing about it.

  So the contract adds: if a worker can state a concrete failure mode, it must (a) fix it, (b) produce an EXECUTED failing case proving it is real and genuinely out of scope, or (c) stop and ask the captain. "Noted but not complete" is not a terminal state. A limitation inherent to the design is a legitimate handoff; a defect the worker could have fixed is not, and the difference is whether anything actually prevented the fix. An agent that can articulate a defect precisely enough to write it down can usually fix it.
...
2. **Mandatory Opus-captain adversarial review (you).** Do NOT trust grok's "done." Review its diff and claims **antagonistically, assuming they are wrong**, and specifically:
  - **Read the worker's Risks / limitations section FIRST, before its summary.** That is where unfixed work goes to look handled. Triage every item into genuine-handoff (inherent to the design, or explicitly the captain's to wire) versus disclosed-and-abandoned (the worker could have fixed it and chose to describe it). Send the second category back, quoting its own words. Do the same to yourself: check whether a risk the worker blamed on a sibling actually reproduces before forwarding it.
  - **Sweep a parameter the worker's tests held fixed. Do not merely re-run them.** Re-running verifies reproducibility, not correctness: a worker tests the scenario it had in mind while writing the code, so the defect lives in the dimension it never varied. Observed: all 23 of a module's own tests passed while its coverage silently saturated at a constant, visible only when the input duration was swept across 0.5/1/2/5/10 s. Identify what is held constant in their fixtures, vary it, and require a parameterised test over that dimension so the same class of defect cannot recur silently.
  - **Doubt your own fixture as hard as you doubt theirs.** An independent scenario is independent, not automatically correct, and a synthetic input that violates a domain constraint manufactures failures in working code. Observed: a captain's scenario injected four times more of an effect than the governing geometry permits, reported a "26% miss", and nearly got correct work rejected; the detector had in fact flagged exactly the physically possible amount. When your measurement contradicts a worker's result, derive the closed-form bound and size your fixture to it BEFORE concluding the code is wrong.
...
> **Reading everything below (Reasoning Effort Policy to Claude Review Standard):** these sections still use **Codex as the historical example** for effort tiers, supervision, watchers, captain-help, and review language. **Codex is DISABLED. cursor-agent usage is EXHAUSTED (2026-09-14). The Mandatory Spawn Path at the top of this file always wins.** Map "Codex" to "the worker"; map `start_visible_codex_*` to `start_visible_grok_worker` / `start_visible_first_mate_grok_pool` (the harness fan-out), `start_visible_agy_worker`, or secondary `start_claude_worker`. Do **not** map Codex fan-out onto `start_visible_cursor_worker` / `start_visible_first_mate_cursor_pool`. Map `steer_visible_codex_run` to `steer_visible_grok_run` / `steer_visible_agy_run` / `steer_claude_run`. Do not call Codex tools because an older paragraph still names them.
...
2. Spawn a **Claude `Agent` subagent** with a compact brief and a built-in `subagent_type`. For independent parallel work, issue several `Agent` calls in one message. Only on an explicit harness request, spawn visible Grok workers (`start_visible_grok_worker` or the Haiku-composed variant, `start_visible_first_mate_grok_pool` for fan-out) and arm `watch_command`, per the Mandatory Spawn Path. Do **not** spawn `start_visible_cursor_worker` / `start_visible_first_mate_cursor_pool` (cursor-agent usage exhausted 2026-09-14).
...
2. **Visible-window workers** - `start_visible_grok_worker` / `start_visible_haiku_composed_grok_worker` for a single worker, `start_visible_first_mate_grok_pool` for structured parallel fan-out; `start_visible_agy_worker` next when grok is capped. Steer with the matching `steer_visible_*_run`. **Never Codex. Never cursor-agent fan-out** (`start_visible_cursor_worker` / `start_visible_first_mate_cursor_pool` are exhausted as of 2026-09-14).
...
- **A launch that returns a run directory has NOT started a worker. Confirm every one is alive before reporting the fleet.** Spawn-succeeded and work-started are different claims. A worker can die inside a second (an unsupported CLI flag is the usual cause), writing nothing, leaving `status: "failed:N"` and an auto-report reading "turn failed before producing a text answer". From the captain's side that is invisible: the fan-out comes back one worker short while every sibling looks healthy. Observed twice in one session, and the owner noticed both times before the captain did. So immediately after launching, check each run's `events.jsonl` is non-trivial and growing and its status is not terminal, and only then describe the fleet as running.
...
- Prefer `start_visible_first_mate_grok_pool` when one coordinator should own the fan-out, and separate visible grok workers when the work items are cleanly file-disjoint. Do **not** use `start_visible_first_mate_codex_pool` (Codex disabled). Do **not** use `start_visible_first_mate_cursor_pool` (cursor-agent usage exhausted 2026-09-14).

## Worker Exhaustion Fallback (down the backend ladder)

When the active worker backend runs out (grok capped, agy buckets cooling, etc.), keep delegating - just move down the ladder (grok-4.6 -> agy -> Claude Sonnet subagents). **Skip cursor-agent:** it is already latched exhausted (2026-09-14). Do not silently start doing all the implementation as the manager model; the point is still to route heavy/parallel work off the manager. The no-nesting / no-parking / flat-fallback rules below are backend-agnostic and apply to every fallback fleet ("Codex" in the detection triggers = the capped backend).
...
> **This is the explicit-request spawn path** (see Subagent Locality and Mandatory Spawn Path): a worker runs in its own visible window with a run directory. Reach for it when the user asks for the multi-agentic harness by name, for visible windows, or for grok agents. Within it, use the **grok CLI** tools (`start_visible_first_mate_grok_pool` / `start_visible_grok_worker`) first, then **agy** if grok is capped. **cursor-agent usage is EXHAUSTED (2026-09-14) - do not fan out with cursor tools.** **The Codex visible tools named in this section are DISABLED - never call them.**
...
- `start_visible_grok_worker` / `start_visible_haiku_composed_grok_worker` / `start_visible_first_mate_grok_pool` / `steer_visible_grok_run` (harness fan-out - documented under "Grok Worker Backend")
- `start_visible_cursor_worker` / `start_visible_haiku_composed_cursor_worker` / `start_visible_first_mate_cursor_pool` / `steer_visible_cursor_run` (EXHAUSTED 2026-09-14 - do not fan out; documented under "Cursor Agent Worker Backend")
...
- an explicit user request for the multi-agentic harness, a visible terminal window, grok agents / grok workers, or cursor-agent / Cursor workers (then `start_visible_grok_worker` / `start_visible_first_mate_grok_pool` with a write sandbox when they asked to edit: do not substitute Claude subagents, do not leave `read-only`, and do **not** fan out to cursor-agent even if they said "cursor")
...
## Cursor Agent Worker Backend (added 2026-08-26; EXHAUSTED 2026-09-14)

**EXHAUSTED (owner 2026-09-14). Do not fan out here.** cursor-agent usage is gone. Do not call `start_visible_cursor_worker` / `start_visible_haiku_composed_cursor_worker` / `start_visible_first_mate_cursor_pool` for harness, visible-window, or parallel fan-out. Use Grok (`start_visible_first_mate_grok_pool` / `start_visible_grok_worker`) instead. These tools remain documented for a possible future revival, or if the owner names cursor anyway after a warning that it will likely fail. Default model is `cursor-grok-4.6-xhigh-fast` (grok 4.6 xhigh Max Mode fast; Cursor Max Mode in `~/.cursor/cli-config.json` unlocks the 1M window). Everyday Claude delegation still uses the `Agent` tool (Subagent Locality): do **not** Bash `cursor-agent -p` from a Claude session.
