...
- **Operational caveats:** the intermittent "malformed HTTP 200" (empty/malformed body) was CLIProxyAPI bug GH#4431 (empty upstream candidate produced a truncated Claude stream), FIXED in CLIProxyAPI v7.2.90+ (we run v7.2.91); keep the proxy at v7.2.90 or newer. Remaining occasional issue is a retryable upstream hang, not the fixed bug. (The agy Claude 4.6 models were dropped because their bucket's 5-hour limit exhausts too fast to be usable - see Quota above.)
