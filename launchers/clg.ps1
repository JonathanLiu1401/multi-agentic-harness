# clg - Claude Code TUI on ANTIGRAVITY (Gemini) models via the local CLIProxyAPI gateway.
#
...
if (-not (Test-Path $KeyFile)) { Write-Error "clg: missing $KeyFile - please save your local client API key from config.yaml to $KeyFile"; exit 1 }
...
$env:CLAUDE_CONFIG_DIR = Join-Path $HOME ".claude-clg"
$env:ANTHROPIC_BASE_URL = $Gateway
$env:ANTHROPIC_AUTH_TOKEN = $Key

$env:ANTHROPIC_MODEL = "gemini-3.8-flash-high(high)"
# The picker's Default row is structural; unset it derived a bogus
# "claude-grok-4.6(xhigh)" id, so point it at a real model.
$env:ANTHROPIC_DEFAULT_MODEL = "gemini-3.8-flash-high(high)"
$env:ANTHROPIC_CUSTOM_MODEL_OPTION = "gemini-3.8-flash-high(high)"
$env:ANTHROPIC_CUSTOM_MODEL_OPTION_NAME = "Gemini 3.8 Flash high"
$env:ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION = "Antigravity Gemini 3.8 Flash via CLIProxyAPI"
...
$env:CLAUDE_CODE_SUBAGENT_MODEL = "gemini-3.8-flash-high(high)"
...
try {
    ...
    Write-Error "clg: gateway not responding at $Gateway"
    Write-Host  "clg: start it with:  Start-ScheduledTask -TaskName CLIProxyAPI"
    Write-Host  "clg: (or run ~\cliproxyapi\start-gateway.ps1 directly)"
    exit 1
}
...
# off. See ~\.cc-bridge\PERF-FINDINGS.md.
